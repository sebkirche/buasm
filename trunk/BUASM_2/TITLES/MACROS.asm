...
; TODO anciennes macros de RosAsm à remplacer par nouvelles Macros C Like
...
;;
[Structure ; TODO à refaire avec enchainement de strutures et un seul sub esp !
...
[Return
    ...
    ExitP] ;  jmp ExitP]                  ; See next equate / Saute à EndP, voir equate suivante


;[ExitP P9>]                     ; Equate to exit proc / Equate pour sortie Proc -> cmp eax &NULL | je ExitP

[ExitP | jmp P9>>] ; TODO quand les If Eslse etc et les sauts courts struturants seront implémentés...
...
[StackOff
    ...
    &3                          ; If &3 >'' restore the registers/variables saved
                                ; Si &3 >'' restaure les registres/variables sauvegardés
...
;;
   ...
   TODO en attente de l'implémentation des sauts courts structurés
