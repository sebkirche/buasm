...
[ErrorOutside:
B$ "An exception occurred outside the Application. 
...
flawlessly without the debugger." EOS

ErrorOutsideTitle: B$ 'External Error' EOS]

[AboutDebugger:
B$ "RosAsm's integrated Win32 debugger.
...
[Smode: B$ '/S' EOS]      ; To run Screen Savers in Saver Mode from the [Run] Button.
...
L0: Mov B$edi 0
    ...
    If eax = &FALSE
        ...
        On eax = &ERROR_DIRECTORY,
            Call 'USER32.MessageBoxA' 0, DebuggeePath, {B$ 'Directory is:' EOS}, &MB_OK
    End_If
        ...
        .If eax = &TRUE
          ...
          ; Prevent buffer overrun
            If D$CallStackEntries >= MAX_CALLSTACK_ENTRIES
                Call 'USER32.MessageBoxA' D$H.DebugDialog, BufferOverrun, {B$ 'Callstack generation' EOS}, &MB_OK+&MB_ICONEXCLAMATION
                Mov eax &FALSE
                ExitP
            End_If
        .End_If
