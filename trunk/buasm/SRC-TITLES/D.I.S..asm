TITLE D.I.S.          ; Version A.Bvvv DD.MM.YY maintainer email and version number go here
...
GetDisPath:
    ...
    Mov esi BUAsmFilesPath, edi PesDisPath
...
LoadPeDisFile:
    ...
    ...If eax <> &INVALID_HANDLE_VALUE
        ...
        Mov D$H.PeDisFile eax

        Call 'KERNEL32.GetFileSize' eax, 0 | Mov D$PeDisFileLength eax
        add eax 10

        Call VirtualAlloc PeDisFileMemory,
                          eax

        add D$PeDisFileMemory 10

        Call 'KERNEL32.ReadFile' D$H.PeDisFile, D$PeDisFileMemory,
                                 D$PeDisFileLength, NumberOfReadBytes, 0

        Call 'KERNEL32.CloseHandle' D$H.PeDisFile
...
L2: If ah > 0F
    ...
    Else_If B$esi > SPC
L8:
        Call 'USER32.MessageBoxA' 0, {B$ 'Bad Hexa Data encounted' EOS},
                                    FindFile.cFileName, 0 ;{B$ 'Reading Pe.dis' EOS}, 0
        While B$esi <> '[' | dec esi | End_While
...
DisToBis:
    ...
    Call 'KERNEL32.FindFirstFileA' PesDisPath, FindFile | Mov D$H.Find eax
...
L1:     Call LoadOneDisFile FindFile.cFileName, &NULL
        ...
        Call VirtualFree PeDisFileMemory

        Call 'KERNEL32.FindNextFileA' D$H.Find FIND_EQU | on eax = &TRUE, jmp L1<

        Call 'KERNEL32.FindClose' D$H.Find
    ...End_If
ret
...
EncodeOneSignature:
  ...
  ; Read the First Byte of the Signature:
    ...
    Else_If al < B$FirstByte
        Call 'USER32.MessageBoxA' 0, {B$ 'Bad order of Records' EOS},
...
L1: .If eax = QUESTIONMARKS
      ; Write the written number of Significative Bytes
        If cl = 0
            Call 'USER32.MessageBoxA' 0, {B$ 'Record beginning with ??' EOS},
                                     FindFile.cFileName, 0
            Mov B$esi+200 0 | ;showme esi
            While D$esi <> 'Id: ' | dec esi | End_While
            Mov B$esi+200 0 | ;showme esi
    ...
    .Else_If eax = CRLF
        If cl <s 1
            Call 'USER32.MessageBoxA' 0, {B$ 'Record ending with ??' EOS},
                                     FindFile.cFileName, 0
            While D$esi <> 'Id: ' | dec esi | End_While
            Mov B$esi+200 0 | ;showme esi
            jmp DisFail
