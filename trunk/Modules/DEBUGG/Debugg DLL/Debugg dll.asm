...
____________________________________________________________________________________________
; EOT
TITLE Debug           ; [VERSION] 23.02.09 remi.coquet@free.fr
____________________________________________________________________________________________
...
;;
     ...
     - A l'initialisations de la dll, [MAIN] reçoit le message &DLL_PROCESS_ATTACH et lance
       commence par lancer la fenêtre d'affichage via le lancement de DEBUGG.EXE. Après
       avoir récupéré le handle de l'edit fils, dimensionné et positionné la fenêtre, ce
       module mets à disposition du PE appelant ces différentes routines. A la fermeture du
       PE, [MAIN] reçoit le message &DLL_PROCESS_DETACH et ferme DEBUGG.EXE
      
       A) [MAIN] reçoit le message &DLL_PROCESS_ATTACH -> initialisation de DEBUGG.EXE

       B) Réception d'un Call du PE par une routine
       
       C) Affichage des contenus dans l'edit de DEBUGG.EXE
       
       D) [MAIN] reçoit le message &DLL_PROCESS_DETACH -> Fermeture de DEBUGG.EXE
 
   [MAIN]
    |
    |--->[DLL_PROCESS_ATTACH] A)
    |    °
    |--->[OutPutStringSimple] B)
    |    |
    |    |--->[FormatOutPut] C)
    |    °    °
    |--->[OutPutString] B)
...
Proc FormatOutPut:
    ...
    ; Formates les chaînes à afficher et les envois à l'edit de DEBUGG.EXE
    _______________________________________________________________________
    ...
    test D@LP.Name NA | jz S1>

    ; Mise en forme du nom dans esi vers edi au format -> [(prefixe Taille)$VAR = ]
    mov esi D@LP.Name
    ...
    ; Complete le buffer avec ' = '
    mov D$edi ' = ' | lea edi D$edi+(3*ASCII)

    ; Complete le buffer avec le contenu
S1: mov esi D@STR.A.Content
    ...
    ; Rajoute en fin de chaîne
    mov B$edi EOS

    ; Envois de la chaîne formatée à la sauvegarde dans DebugLog.txt
    Call OutPutLogFile @STR.A.Out

    ; Envois de la chaîne formatée à l'edit de DEBUGG.EXE pour affichage
    Call 'USER32.SendMessageA' D$H.Edit,
                               &EM_REPLACESEL,
                               &FALSE,
                               @STR.A.Out
...
Proc OutPutStringSimple::
    _________________________________

    ; Affiche les chaînes transmises
    _________________________________

    Argument @STR.A.Content

    pushad

        Call FormatOutPut &NULL,
                          D@STR.A.Content
...
Proc OutPutHex::
    ...
    ; Affiche le nom du Lpointeur et son contenu au format hexadecimal
    ___________________________________________________________________

    Arguments @LP.Name,
              @STR.A.Content

    [@STR.A.Hexprint: B$ '         Hex' EOS]

    pushad

        Mov ebx D@STR.A.Content, ecx 7 | lea edi D@STR.A.Hexprint+ecx

        std

        L0: mov al bl | and al 0F

                On al => 0A add al 7

                add al '0' | stosb | shr ebx 4
...
Proc OutPutDec::
    ...
    ; Affiche le nom du Lpointeur et son contenu au format entier non signé
    ________________________________________________________________________

    Arguments @LP.Name,
              @STR.A.Content

    Structure @STR.A.Buffer,
    &MAX_PATH .ASCII

    pushad

        mov edi @STR.A.Buffer, eax D@STR.A.Content

        push NA

        mov ecx 10

L1:     xor edx edx | div ecx | push edx | test eax eax | jnz L1<

L2:     pop eax | test eax eax | js S0>
...
Proc OutPutSignedDec::
    ____________________________________________________________________

    ; Affiche le nom du Lpointeur et son contenu au format entier signé
    ____________________________________________________________________
    ...
    pushad
        ...
        neg eax | mov B$edi '-'  | jmp S2>

S1:     mov B$edi '+'

S2:     lea edi D$edi+(1*ASCII)
...
Proc OutPutError::
    ...
    ; Affiche le message correspondant au dernier N° d'erreur dans la langue de l'OS cible
    _______________________________________________________________________________________
    ...
    pushad
        ...
        Call 'KERNEL32.LocalFree' D@LP.Buf.Buffer
...
Proc OutPutLogFile::
    ...
    ; Ajoute dans le ficher DebugLog la chaîne transmise
    _____________________________________________________

    Argument @STR.A.ASCII
    ...
    pushad
        ...
        Call 'KERNEL32.SetFilePointer' D$H.File,
                                       &NULL,
                                       &NULL,
                                       &FILE_END
        ...
        Else
            ...
            Call 'KERNEL32.lstrlenA' D@STR.A.ASCII

            Call 'KERNEL32.WriteFile' D$H.File,
                                      D@STR.A.ASCII,
                                      eax,
                                      @LP,
                                      &NULL
...
Proc OutPutFloat::
    ...
    ; Affiche le nom du Lpointeur et son contenu au format float
    _____________________________________________________________
   ...
   (32*ASCII) .ASCII
    ...
    pushad
        ...
        ; Vide le buffer
        mov eax @STR.A, ecx (@STR.ASIZE/DWORD)

        L0: mov D$eax+ecx*DWORD EOS | sub ecx 1 | jns L0<

        ; Retire le réel de la pile FPU
        fstp R@R.LSD

        ; Réel -> ASCII buffer
        CCall 'MSVCRT.sprintf' @STR.A,
                               {"%+.10f" EOS},
                               D@R.LSD,
                               D@R.MSD
...
Proc OutPutReel::
    ...
    ; Affiche le nom du Lpointeur et son contenu au format réel
    ____________________________________________________________
   ...
   (32*ASCII) .ASCII
    ...
    pushad
        ...
        ; Vide le buffer
        mov eax @STR.A, ecx (@STR.ASIZE/DWORD) | L0: mov D$eax+ecx*DWORD EOS | sub ecx 1 | jns L0<
...
Proc VideDebbug::
    ...
    ; Vide l'edit de son contenu courant mais n'efface pas le fichier DebugLog.txt
    _______________________________________________________________________________
...
;;
    ...
    Mov eax D@H.Instance, D$H.Instance eax

    mov eax D@Reason | Comp eax &DLL_THREAD_DETACH > S1>

    Call D@TABLE.LP.Reason+eax*DWORD
...
Proc DLL_PROCESS_ATTACH:
    ____________________________________________________________________________________

    ; Initialisation du MODULE: Lance DEBUGG.EXE et récupère le handle de son edit fils
    ____________________________________________________________________________________

    Structure @STR.A.CurrentDirectory,
    &MAX_PATH .Datas

    Structure @STARTUPINFO, ; Pas de préfixe et en majuscules pour une structure de l'OS
        DWORD .cb,
        DWORD .lpReserved,
        DWORD .lpDesktop,
        DWORD .lpTitle,
    ...
    pushad
        ...
        Move D@STARTUPINFO.cb @STARTUPINFO_SIZE
        Mov D@STARTUPINFO.lpReserved eax,
            D@STARTUPINFO.lpDesktop eax,
            D@STARTUPINFO.lpTitle eax,
            D@STARTUPINFO.dwX 16,
            D@STARTUPINFO.dwY 48,
            D@STARTUPINFO.dwXSize 448,
            D@STARTUPINFO.dwYSize 320,
            D@STARTUPINFO.dwXCountChars eax,
            D@STARTUPINFO.dwYCountChars eax,
            D@STARTUPINFO.dwFillAttribute eax,
            D@STARTUPINFO.dwFlags &STARTF_USEPOSITION+&STARTF_USESIZE,
            W@STARTUPINFO.wShowWindow ax,
            W@STARTUPINFO.cbReserved2 ax,
            D@STARTUPINFO.lpReserved2 eax,
            D@STARTUPINFO.hStdInput eax,
            D@STARTUPINFO.hStdOutput eax,
            D@STARTUPINFO.hStdError eax

        Call 'KERNEL32.CreateProcessA' {B$ 'Debugg.exe' EOS},
                                       eax,
                                       eax,
                                       eax,
                                       eax,
        ...
        Call 'KERNEL32.Sleep' 50

L1:     Call 'USER32.FindWindowA' STR.A.WindowClass,
                                  STR.A.WindowClass

        test eax eax | jz L1<

L2:     Call 'USER32.FindWindowExA' eax,
                                    &NULL,
                                    {B$ 'EDIT' EOS},
                                    &NULL
        ...
        Call 'KERNEL32.GetCurrentDirectoryA' &MAX_PATH,
                                             @STR.A.CurrentDirectory

        Call 'KERNEL32.lstrcat' @STR.A.CurrentDirectory,
                                {B$ '\DebugLog.txt' EOS}

        mov edx &CREATE_ALWAYS

L3:     Call 'KERNEL32.CreateFileA' @STR.A.CurrentDirectory,
        ...
        Comp eax &SV_FILE_ALREADY_EXISTS = L3<

        mov D$H.File eax

    popad

    ; Affiche le numero de version de la dll
    @VERSION: Call OutPutStringSimple {B$ CR LF "Version: A0.B1 R02"  CR LF EOS}

EndP
____________________________________________________________________________________________

Proc DLL_PROCESS_DETACH:
    _______________________________

    ; Referme le module DEBUGG.EXE
    _______________________________
    ...
    pushad
        ...
        mov D@LP.ExitCode &TRUE

        Call 'KERNEL32.CloseHandle' D$H.File

        Call 'KERNEL32.TerminateProcess' D$PROCESS_INFORMATION@H_Process,
                                         @LP
...
; W:
; Si vous changez le nom ici, il faut que vous le changiez aussi dans DEBUGG.EXE
[<4 STR.A.WindowClass: B$ 'DEBUGG' EOS]
...
; Structures Microsoft -> Pas de préfixe
; P:
[PROCESS_INFORMATION:
 @H_Process: ?
 @H_Thread: ?
